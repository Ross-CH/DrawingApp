...
//V08:
//Attempt to move the collider to the start position so that it activates the redraw and fadeout colloders at the right time. - 

using UnityEngine;
using System;


public class RecordObjectMovementV08 : MonoBehaviour
{
...
//_
    ...
    void ResetColliderPosition()
    {
        ...
        var CurLineName = ListOfLines[CurrentLineNum - 1];
        CurrentLineObject = GameObject.Find(CurLineName);
        TrailRenderer Trail = CurrentLineObject.GetComponent<TrailRenderer>();
        ...
        while (CurrentWaypoint < (NumVerts - 1))
        {
            var Vector3Val = CurrentLineVertices[(NumVerts-1) - CurrentWaypoint];

            float Xpos = Vector3Val.x;
            float Ypos = Vector3Val.y;
            float Zpos = Vector3Val.z;

            CurrentLineAttributes.Vertices[CurrentWaypoint].Set(Xpos, Ypos, Zpos);
            CurrentWaypoint++;
        }
        //_
